...
# Get all ADO work items to create a pool for fuzzy matching
print("Fetching all ADO work items for fuzzy matching...")
all_ado_work_items = {}

try:
    # Query for recent work items (last 90 days)
    wiql_query = {
        "query": f"SELECT [System.Id], [System.Title] FROM WorkItems WHERE [System.TeamProject] = '{ADO_PROJECT}' AND [System.CreatedDate] >= @Today - 90 ORDER BY [System.CreatedDate] DESC"
    }

    wiql_url = f"{ADO_API_BASE}/wit/wiql?api-version=5.0"
    wiql_response = requests.post(wiql_url, auth=HTTPBasicAuth('', ADO_PAT), json=wiql_query, timeout=30)
    ...
    if wiql_response.status_code == 200:
        ...
        # Fetch details for these work items (limit to first 200 to avoid timeout)
        for ado_id in work_item_ids[:200]:
            if ado_id not in ado_work_items:  # Don't re-fetch already fetched items
                try:
                    url = f"{ADO_API_BASE}/wit/workitems/{ado_id}?api-version=5.0"
                    response = requests.get(url, auth=HTTPBasicAuth('', ADO_PAT), timeout=10)
        ...
        print(f"Could not fetch ADO work items for fuzzy matching: HTTP {wiql_response.status_code}")
except Exception as e:
    print(f"Error during fuzzy matching setup: {str(e)}")
    all_ado_work_items = {}
...
if len(all_ado_work_items) > 0 and len(unlinked_items) > 0:
    print(f"\nAnalyzing {len(unlinked_items)} unlinked Jira items for potential matches...")

    # Create a dictionary of ADO titles for fuzzy matching
    ...
    for idx, jira_row in unlinked_items.iterrows():
        ...
        # Only accept matches with score >= 70 (confidence level >= Medium)
        for match in matches:
            ...
            if score >= 70 and ado_id_key:  # Minimum 70% match required
                # Confidence levels
                if score >= 90:
...
# Create DataFrame for fuzzy matches
...
# === SHEET 6: POTENTIAL MATCHES (FUZZY MATCHING) ===
...
if len(df_fuzzy_matches) > 0:
    ...
    # Format header
    for cell in ws_fuzzy[1]:
        cell.font = Font(bold=True, color="FFFFFF")
        cell.fill = PatternFill(start_color="FFA500", end_color="FFA500", fill_type="solid")
        cell.alignment = Alignment(horizontal="center")

    # Auto-adjust columns
    for column in ws_fuzzy.columns:
        max_length = 0
        column_letter = column[0].column_letter
        for cell in column:
            try:
                ...
                pass
        adjusted_width = min(max_length + 2, 50)
        ws_fuzzy.column_dimensions[column_letter].width = adjusted_width

    # Add instructions
    ws_fuzzy.insert_rows(1, 3)
    ws_fuzzy["A1"] = "Potential Matches Based on Title Similarity"
    ws_fuzzy["A1"].font = Font(size=14, bold=True, color="FFFFFF")
    ws_fuzzy["A1"].fill = PatternFill(start_color="FFA500", end_color="FFA500", fill_type="solid")
    ws_fuzzy["A2"] = "Suggested matches using fuzzy text matching (Score >= 70%). Only Medium, High, and Very High confidence matches are shown."
    ws_fuzzy["A2"].font = Font(italic=True)
    ws_fuzzy.merge_cells('A2:I2')
else:
    ws_fuzzy.append(["No potential matches found"])
    ws_fuzzy.append(["All unlinked Jira items have no similar titles in Azure DevOps work items."])

# === SHEET 7: UNLINKED ISSUES ===
...
print(f"  Potential Matches Found (Fuzzy): {len(fuzzy_matches)}")
